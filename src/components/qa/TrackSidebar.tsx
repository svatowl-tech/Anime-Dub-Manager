...
export const TrackSidebar: React.FC<TrackSidebarProps> = ({
  ...
  return (
    <div className="w-80 border-r border-neutral-800 flex flex-col">
      ...
      {/* Gap Detection Quick Action Bar */}
      {onOpenGapDetection && (
        <div className="px-3 py-2 bg-amber-950/20 border-b border-amber-900/30 flex items-center justify-between">
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded-md text-[11px] font-bold transition-all flex items-center gap-1.5 border shadow-sm ${
              detectedGapsCount > 0
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-2">
        ...
        {tracks.map(track => {
          ...
          const trackGaps = gapsByTrack[track.id] || 0;
            ...
            >
              <div className="flex justify-between items-start mb-1">
                <div className="flex items-center gap-1.5">
                  ...
                  {trackGaps > 0 && (
                    <span 
                      className="text-[9px] font-bold px-1.5 py-0.2 rounded bg-amber-500/20 text-amber-300 border border-amber-500/40"
                      title={`Внимание: обнаружено ${trackGaps} пропущенных реплик субтитров`}
                    >
                      {trackGaps} проп.
                    </span>
                  )}
